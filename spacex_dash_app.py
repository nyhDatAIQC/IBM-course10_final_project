...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['class'].replace({1:'success', 0:'failed'},inplace=True)
        title_site = 'Total Success Launches for site ' + entered_site
...
def get_scatter_chart(input_site, input_range):
    filtered_df = spacex_df
    ...
    if input_site == 'ALL':
        ...
        title_site = 'Correlation between payload and success for ' + input_site
        mask = (filtered_df['Payload Mass (kg)'] >= low) & (filtered_df['Payload Mass (kg)'] <= high)
        fig = px.scatter(filtered_df[mask], x='Payload Mass (kg)', 
                    y='class', color="Booster Version Category",
                    title=title_site)
